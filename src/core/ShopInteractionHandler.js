...
export async function handleButton(interaction) {
  ...
  if (!customId || !customId.startsWith('shop_')) {
    return false;
  }
...
async function handleShopCategory(interaction, category) {
  try {
    ...
    console.error('[ShopInteractionHandler] handleShopCategory error:', error);
    await interaction.editReply({ content: '❌ Failed to load category items.', embeds: [], components: [] });
  }
}
...
async function handleShopPurchase(interaction, itemId) {
  try {
    ...
    console.error('[ShopInteractionHandler] handleShopPurchase error:', error);
    await interaction.editReply({ content: '❌ An error occurred during purchase.', embeds: [], components: [] });
  }
}
...
async function handleShopBack(interaction) {
  try {
    ...
    console.error('[ShopInteractionHandler] handleShopBack error:', error);
    await interaction.editReply({ content: '❌ Failed to return to shop.', embeds: [], components: [] });
  }
}
